...
public boolean insertUser(final Userinfo userinfo) {
		...
		int id =(int) userCollection.count()+1;
...
public class UserinfoDao {
	...
	//Check if login information correct
	public boolean checkLogin(Userinfo user) throws Exception {
		...
		try {
			...
			String sql = "SELECT id FROM userinfo WHERE username=? AND password=? AND usertype=?";
	...
	public String getFirstName(Userinfo user) {
		...
		try {
			...
			String sql = "SELECT fname FROM userinfo WHERE username=?";
			pstmt = conn.prepareStatement(sql);
			String username = user.getUsername();
			pstmt.setString(1, username);
			ResultSet rs = pstmt.executeQuery();
	...
	public String getCity(Userinfo user) {
		...
		try {
			...
			String sql = "SELECT city FROM userinfo WHERE username=?";
			pstmt = conn.prepareStatement(sql);
			String username = user.getUsername();
			pstmt.setString(1, username);
			ResultSet rs = pstmt.executeQuery();
	...
	public boolean insertUser(Userinfo user) {
		...
		try {
			...
			String sql = "INSERT INTO userinfo(usertype,username,password,fname,lname,picpath,gender,level,dob,city) VALUES(?,?,?,?,?,?,?,?,?,?)";
			pstmt = conn.prepareStatement(sql);
			...
			pstmt.setString(6, user.getPicpath());
	...
	//Get next id from table Userinfo
	public String getNextId() {
		...
		String sql = "SELECT AUTO_INCREMENT FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'peakcentredb' AND TABLE_NAME = 'userinfo'";
		try {
			pstmt = conn.prepareStatement(sql);
			ResultSet rs = pstmt.executeQuery();
			if (rs.next()) {
	...
	//Check if first name and last name exists in table userinfo
	public boolean checkUserExists(String fname, String lname) {
		dbc = new DatabaseConnection();
		conn = dbc.getConnection();
		PreparedStatement pstmt = null;
		...
		try {
			...
			String sql = "SELECT id FROM userinfo WHERE fname=? AND lname=?";
			pstmt = conn.prepareStatement(sql);
	...
	//Check if user exist from one city
	public boolean checkUserExistsWithUsertype(String fname, String lname,
			String city) {
		dbc = new DatabaseConnection();
		conn = dbc.getConnection();
		PreparedStatement pstmt = null;
	...
	//Get User information from table userinfo
	public ArrayList<Userinfo> getUserinfo(String fname, String lname) {
		dbc = new DatabaseConnection();
		conn = dbc.getConnection();
		PreparedStatement pstmt = null;
		...
		try {
			...
			String sql = "SELECT usertype,username,fname,lname,picpath,gender,level,dob,city,password,id FROM userinfo WHERE fname=? AND lname=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, fname);
			pstmt.setString(2, lname);
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				...
				ui.setUsertype(rs.getString(1));
				ui.setUsername(rs.getString(2));
				ui.setFname(rs.getString(3));
				ui.setLname(rs.getString(4));
				ui.setPicpath(rs.getString(5));
				ui.setGender(rs.getString(6));
				ui.setLevel(rs.getString(7));
				ui.setDob(rs.getString(8));
				ui.setCity(rs.getString(9));
				ui.setPassword(rs.getString(10));
				ui.setId(rs.getInt(11));
				list.add(ui);
			}
			conn.commit();
			conn.setAutoCommit(autoCommit);
	...
	public boolean updateUserinfo(Userinfo user) {
		...
		try {
			...
			String sql = "UPDATE userinfo SET usertype=?,username=?,password=?,fname=?,lname=?,gender=?,level=?,dob=?,city=? WHERE id=?";
			pstmt = conn.prepareStatement(sql);
			...
			pstmt.setString(7, user.getLevel());
